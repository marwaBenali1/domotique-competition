...
import { lightingSelectors, changeMode } from "../redux/LightingSlicer";
...
export default function Lighting(props) {
    ...
    const activeMode = useSelector(lightingSelectors.activeMode);
    ...
    return(
        ...
        {/* button for modes */}
        <div className="row p-0 justify-content-center mt-1"> 
            {arrIcons.map((item, index) => {
                return <IconButton 
                            key={`${index}-lighting-button`} 
                            name={item.name} 
                            icon={item.icon}
                            isOutlined={item.name === activeMode}
                            onClickHandler={() => dispatch(changeMode(item.name))}
                            />
            })}      
        </div>
        ...
        <div
            ...
            style={{ 
                ...
                pointerEvents: (activeMode === "Mode manuel") ? "" : "none",
                opacity: (activeMode === "Mode manuel") ? 1 : 0.2
              }}
...
function IconButton(props) {
    const { name, icon, isOutlined, onClickHandler } = props;
